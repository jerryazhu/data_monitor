...
public class IosServiceImpl implements IosService{
    ...
    public LinkedHashMap<String, String> getIosApp() {
        ...
        Query q = entityManager.createNativeQuery("select app_version as name,count(app_version) as count from ( select * from ABC360_IOS_APP_DEVICE_TBL where time > (UNIX_TIMESTAMP(now())- 3600*24*30)) a inner JOIN\n" +
    ...
    public LinkedHashMap<String, String> getIosSystem() {
        LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
        Query q = entityManager.createNativeQuery("select os_version as name,count(os_version) as count from ( select * from ABC360_IOS_APP_DEVICE_TBL where time > (UNIX_TIMESTAMP(now())- 3600*24*30)) a inner JOIN\n" +
                "(select uid,max(time) as time from ABC360_IOS_APP_DEVICE_TBL where time > (UNIX_TIMESTAMP(now())- 3600*24*30) group by uid) b\n" +
                "on a.time = b.time and a.uid = b.uid \n" +
                " GROUP BY (os_version)");
        List<Object[]> list = q.getResultList();
