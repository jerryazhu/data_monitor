...
public class AndroidAppServiceImpl implements AndroidAppService{
    ...
    public LinkedHashMap<String, String> getAndroidApp() {
        ...
        Query q = entityManager.createNativeQuery("select app_version as name,count(app_version) as count from ( select * from ABC360_ANDROID_APP_DEVICE_TBL where time > (UNIX_TIMESTAMP(now())*1000 - 3600*24*10*1000)) a inner JOIN\n" +
