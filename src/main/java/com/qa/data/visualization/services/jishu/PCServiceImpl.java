...
public class PCServiceImpl implements PCService {
    ...
    public LinkedHashMap<String, String> getTchPCApp() {
        LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
        Query q = entityManagerSec.createNativeQuery("select version as name,count(version) as count from (select * from ebk_pc_tch_client where version !='UnKnow version' and login_time > (UNIX_TIMESTAMP(now())- 3600*24*1)) a inner JOIN\n" +
    ...
    public LinkedHashMap<String, String> getTchPCSystem() {
        LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
        Query q = entityManagerSec.createNativeQuery("select os as name,count(os) as count from (select * from ebk_pc_tch_client where os!='Unknow os' and login_time > (UNIX_TIMESTAMP(now())- 3600*24*1)) a inner JOIN\n" +
